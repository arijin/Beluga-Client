...
void UsvWindPlugin::Load(physics::WorldPtr _parent, sdf::ElementPtr _sdf) {
  if (char* env_dbg = std::getenv("VRX_DEBUG")) {
    ...
    if (std::string(env_dbg) == "false") this->debug = false;
  } else {
  ...
  if (!_sdf->HasElement("wind_obj") || !_sdf->GetElement("wind_obj")) {
  ...
  if (_sdf->HasElement("update_rate")) {
    this->updateRate = _sdf->GetElement("update_rate")->Get<double>();
  }
...
void UsvWindPlugin::Update() {
  ...
  if (!this->windObjsInit) {
    ...
    if (windObjsInitCount == windObjs.size()) this->windObjsInit = true;
  }
#if GAZEBO_MAJOR_VERSION >= 8
  ...
  for (auto& windObj : this->windObjs) {
    ...
    // initialized
    if (!windObj.init || !windObj.link) continue;
